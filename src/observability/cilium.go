...
func convertFlowLogToCiliumLog(flowLog *flow.Flow) types.CiliumLog {
...
func ProcessCiliumFlow(flowLog *flow.Flow) {
	...
	locNetLog := types.CiliumLog{}

	netLog := convertFlowLogToCiliumLog(flowLog)
	if netLog == locNetLog {
		log.Error().Msg("cilium flow is empty")
	}
	...
	if isEntryExist {
		if err := libs.UpdateCiliumLogsMySQL(CfgDB, netLog); err != nil {
			log.Error().Msg(err.Error())
		}
	} else {
		if err := libs.InsertCiliumLogsMySQL(CfgDB, netLog); err != nil {
			log.Error().Msg(err.Error())
		}
	}
}
